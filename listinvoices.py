...
List invoices from QBO.



Usage:
  listinvoices.py  [options]
...
Options:
  ...
  --and-then-delete        Remove the invoices revealed by this query.
...
def invoice_iterable(whereclause="",**kwargs):
    reps = 0
    yielded = 0
    
    while (True):
        
        invoices = Invoice.where(whereclause,
            qb=qbo_client,
            max_results=100,
...
def main():
    ...
    ilist = pd.DataFrame( [ invoice2row(inv) for inv in invoice_iterable(whereclause=whereclause)])
    if (len(ilist) == 0 ) :
    ...
    if (arguments['--and-then-delete'] is True):
        if ( arguments['--doit'] is True):
            ...
            invoices = Invoice.where(whereclause,qb=qbo_client)
            results = batch_delete(invoices,qb=qbo_client)
